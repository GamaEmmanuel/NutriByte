...
public class NutriProfiler {
	...
	public static ObservableList<RecommendedNutrient> recommendedNutrientsList = FXCollections.observableArrayList();  
	...
	static void createNutriProfile(Person person) {
		...
		//	recommendedNutrientsList.clear();
			...
			RecommendedNutrient energyRN = new RecommendedNutrient(ENERGY_NUTRIENT_CODE, energy);
			recommendedNutrientsList.add(energyRN);
			
			float []nutriRequirements = person.calculateNutriRequirement(); 

			for (int i = 0; i < nutriRequirements.length; i++) {
				RecommendedNutrient tempList = new RecommendedNutrient(NutriProfiler.NutriEnum.values()[i].getNutrientCode(), nutriRequirements[i]);
				recommendedNutrientsList.add(tempList);
				}
